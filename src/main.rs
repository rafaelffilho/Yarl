...
impl Object {
	...
	pub fn attack(&mut self, target: &mut Object, game: &mut Game) {
		...
		if damage > 0 {
			...
			game.messages.add(
				format!("{} attacks {} but has no effects", self.name, target.name),
				WHITE,
			);
		}
	}
...
impl Rect {
	...
	pub fn intersercts_with(&self, other: &Rect) -> bool {
...
fn make_map(objects: &mut Vec<Object>) -> Map {
	...
	for _ in 0..MAX_ROOMS {
		...
		let failed = rooms
			.iter()
			.any(|other_room| new_room.intersercts_with(other_room));
	...
	for _ in 0..num_monster {
		...
		if !is_blocked(x, y, map, objects) {
			let mut monster = if rand::random::<f32>() < 0.8 {
				...
				let mut troll = Object::new(x, y, 'T', "troll", DARK_GREEN, true);
	...
	for _ in 0..num_items {
		...
		if !is_blocked(x, y, map, objects) {
			...
			let item = if dice < 0.7 {
				...
				let mut object = Object::new(x, y, '#', "scroll of ligthning bolt", LIGHT_YELLOW, false);
...
fn use_item(inventory_id: usize, tcod: &mut Tcod, game: &mut Game, objects: &mut [Object]) {
	...
	if let Some(item) = game.inventory[inventory_id].item {
		...
		match on_use(inventory_id, tcod, game, objects) {
			...
			UseResult::Cancelled => {
				game.messages.add("Cancelled", RED);
			}
		}
	} else {
		game.messages.add(
...
fn cast_confuse(
	...
	if let Some(monster_id) = monster_id {
		...
		game.messages.add(
			format!(
				"The eyes of {} look vacant, as ge starts to stumble around",
		...
		game.messages.add("No enemys is close enough", RED);
		UseResult::Cancelled
	}
}
...
fn cast_lightning(
	...
	if let Some(monster_id) = monster_id {
		...
		game.messages.add("No enemy close enough to strik", RED);
		UseResult::Cancelled
	}
}
...
fn menu<T: AsRef<str>>(header: &str, options: &[T], width: i32, root: &mut Root) -> Option<usize> {
	assert!(
		options.len() <= 26,
		"Cannot have a menu with more then 26 options"
	);
...
fn pick_item_up(object_id: usize, game: &mut Game, objects: &mut Vec<Object>) {
	if game.inventory.len() >= 26 {
		game.messages.add(
			format!(
				"Your inventory is full! Cannot pickup {}.",
...
fn main() {
	...
	game
		.messages
		.add("Welcome stranger! Prepare to perish", RED);
	...
	while !tcod.root.window_closed() {
		...
		if objects[PLAYER].alive && player_action != PlayerAction::DidntTookTurn {
